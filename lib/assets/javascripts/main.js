...
function WJ_Validator(args,css_framework,log){
	...
	***/
	this.show_invalid_errors = function(deferred_arr,ret_val,e){
		console.log("came to show invalid errors with:");
		console.log(e);
				...
				_.each([event,"custom"],function(ev){
					_.each(event_handlers[event],function(id){
						...
						if($(id + ":not(." + ev + "_bound)").length){
							console.log("event:" + ev + "not yet bound to id:" + id);
							$(id).addClass(ev + "_bound");
							$(id).on(ev,function(e,ret){
								if(ev == "custom" && id == "#user_login"){
									console.log("triggered custom on user login");
								}
								if(ret){
		...
		***/
		_.each(Object.keys(_this.args),function(fo){
			...
			$(document).on(event_name,"#" + fo,function(e,ret){
				if(_.isEmpty(ret)){
					console.log("---BLANKING OUT ON RETURN TRUE");
					return true;
				}
				var defs = [];
				var results = {};
